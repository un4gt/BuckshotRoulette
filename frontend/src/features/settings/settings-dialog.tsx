...
export function SettingsDialog({ open, onOpenChange }: SettingsDialogProps) {
  const {
    ...
    setDemonAISource,
    setPlayerAISource,
    setDemonPresetModelId,
    setPlayerPresetModelId,
  } = useGameSettingsStore()
  const { isAuthenticated } = useAuthStore()
  const [activeTab, setActiveTab] = useState<'general' | 'demon' | 'player'>('general')

  // 是否有可用的预配置模型
  const hasPresetModels = settings.availablePresetModels.length > 0
        ...
        <div className="min-h-[300px] py-4">
          ...
          {activeTab === 'demon' && (
            <AIConfigForm
              ...
              source={settings.demonAISource}
              presetModelId={settings.demonPresetModelId}
              presetModels={settings.availablePresetModels}
              showPresetOption={isAuthenticated && hasPresetModels}
              onConfigChange={setDemonAI}
              onSourceChange={setDemonAISource}
              onPresetModelChange={setDemonPresetModelId}
            />
          )}
          ...
          {activeTab === 'player' && settings.gameMode === 'ai-vs-ai' && (
            <AIConfigForm
              ...
              source={settings.playerAISource}
              presetModelId={settings.playerPresetModelId}
              presetModels={settings.availablePresetModels}
              showPresetOption={isAuthenticated && hasPresetModels}
              onConfigChange={setPlayerAI}
              onSourceChange={setPlayerAISource}
              onPresetModelChange={setPlayerPresetModelId}
            />
          )}
        </div>
...
interface AIConfigFormProps {
  ...
  presetModels: { id: string; name: string; provider: AIProvider; description?: string }[]
  showPresetOption: boolean
  onConfigChange: (config: Partial<AIConfigFormProps['config']>) => void
  onSourceChange: (source: AISource) => void
  onPresetModelChange: (id: string | null) => void
}
...
function AIConfigForm({
  ...
  const selectedPresetModel = presetModels.find(m => m.id === presetModelId)
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 预配置模型选择 */}
      {source === 'preset' && showPresetOption ? (
        <div className="space-y-4">
          <Select
            id="presetModel"
            label="选择模型"
            value={presetModelId || ''}
            onChange={(e) => onPresetModelChange(e.target.value || null)}
            options={[
              { value: '', label: '请选择模型...' },
              ...presetModels.map(m => ({
                value: m.id,
          ...
          <div className="flex items-center gap-2 text-xs">
            <span
              className={`h-2 w-2 rounded-full ${
                presetModelId ? 'bg-green-500' : 'bg-red-500'
              }`}
            />
            <span className="text-muted-foreground">
              {presetModelId
                ? `已选择: ${selectedPresetModel?.name || presetModelId}`
                : '请选择一个预配置模型'}
            </span>
          </div>
        </div>
