      ...
      try {
        ...
        await conversation.chatStream(queryPrompt, {
          ...
          onComplete: async (output: AgentTurnOutput) => {
            ...
            // 1. 思考展示延迟：基于文本长度动态计算（1000-3000ms）
            const thoughtDelay = Math.min(3000, Math.max(1000, output.thought.length * 30))
            await new Promise((resolve) => setTimeout(resolve, getDelayTime(thoughtDelay)))

            // 2. 对话展示延迟：基于文本长度动态计算（800-2000ms）
            const dialogueDelay = Math.min(2000, Math.max(800, output.dialogue.length * 40))
            await new Promise((resolve) => setTimeout(resolve, getDelayTime(dialogueDelay)))
            ...
            // 3. 决策展示延迟：固定 1000ms，让用户看清"AI 决策: xxx"
            await new Promise((resolve) => setTimeout(resolve, getDelayTime(1000)))
            ...
            // 4. 动作完成后延迟：800ms
            await new Promise((resolve) => setTimeout(resolve, getDelayTime(800)))
          },
          onError: (error) => {
